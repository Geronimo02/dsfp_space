  ...
  const renderNavItem = (item: NavItem, isChild = false, isFavoritesList = false) => {
      ...
      return (
        ...
        >
          <CollapsibleTrigger
            ...
            aria-label={`${item.title} - ${isOpen ? 'Expandido' : 'Colapsado'}`}
          >
            <div className="flex items-center gap-2.5 flex-1">
              <Icon className="w-4 h-4 shrink-0" aria-hidden="true" />
              <span className="truncate text-sm">{item.title}</span>
              {item.badge && item.badge > 0 && (
                <Badge variant="destructive" className="ml-auto text-[10px] h-4 px-1" aria-label={`${item.badge} notificaciones`}>
                  {item.badge}
                </Badge>
              )}
            </div>
    ...
    return (
      ...
      >
        ...
        {item.badge && item.badge > 0 && (
          <Badge variant="destructive" className="text-[10px] h-4 px-1" aria-label={`${item.badge} notificaciones`}>
            {item.badge}
          </Badge>
        )}
        <Button
    ...
    <UISidebar collapsible="offcanvas" className="border-r border-sidebar-border w-64">
      <div className="flex flex-col h-full bg-gradient-to-b from-sidebar to-sidebar/95">
        ...
        <div className="px-3 py-2 border-b">
          <div className="relative">
            ...
            <Input
              ...
              autoFocus={false}
              readOnly={false}
              aria-label="Buscar módulos en el menú"
              role="searchbox"
              onFocus={(e) => {
        ...
        {favoriteItems.length > 0 && !searchQuery && (
          <div className="px-3 py-2 border-b" role="region" aria-label="Módulos favoritos">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-4 sidebar-scroll" aria-label="Menú principal">
        ...
        {!isPlatformAdmin && (
          <div className="px-3 py-2 border-t">
            <Button
              ...
              aria-label="Ver más funcionalidades disponibles"
            >
