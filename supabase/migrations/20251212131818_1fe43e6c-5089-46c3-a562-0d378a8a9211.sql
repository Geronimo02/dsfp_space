...
DROP FUNCTION IF EXISTS public.calculate_subscription_price(UUID, TEXT, INTEGER);

-- Recreate as a function returning JSONB instead of TABLE
...
-- Update the trigger function to use the new JSONB return type
CREATE OR REPLACE FUNCTION public.update_subscription_price()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  v_price_data := calculate_subscription_price(NEW.company_id, 'monthly', 0);
  
  -- Update company subscription with calculated values
  UPDATE public.company_subscriptions 
  SET 
    calculated_price = COALESCE((v_price_data->>'total_price')::decimal, 0),
