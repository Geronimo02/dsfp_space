...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Try to get existing intent, or create a new one if combining Step 4+5
    let intent: any = null;
    let intentErr: any = null;
    
    const { data: existingIntent, error: checkErr } = await supabaseAdmin
      .from("signup_intents")
      .select("*")
      .eq("id", intent_id)
      .maybeSingle();

    if (!checkErr && existingIntent) {
      intent = existingIntent;
    } else {
      // Intent doesn't exist - this is okay when Step 4 and 5 are combined
      // We'll look for the payment method by plan_id and email
      console.log("[finalize-signup] Intent not found, will look for payment method by plan_id");
    }
    ...
    // =================================================================
    // Use intent.email if available, otherwise will fetch from payment method
    ...
    if (spmErr) {
      console.error("[finalize-signup] Error fetching payment method:", spmErr);
      return json({ error: "Error al obtener método de pago" }, 500);
    }

    if (!spm) {
      console.error("[finalize-signup] No payment method found");
      return json({ error: "No se encontró método de pago. Por favor, vuelve al paso anterior." }, 404);
    }

    // Now we have spm, extract email from it if intent didn't have it
    const actualEmail = intent?.email || spm.email;
    ...
    const finalPlanId = !intent || intent.plan_id === FREE_PLAN_ID 
      ? BASIC_PLAN_ID 
      : (intent.billing_plan_id ?? intent.plan_id ?? spm.plan_id);
    ...
    const companyName = intent?.company_name ?? spm.company_name ?? fullName;
    ...
    try {
      ...
      if (userErr) {
        ...
        if (msg.toLowerCase().includes("already") || msg.toLowerCase().includes("registrado")) {
          ...
          const existing = usersList?.users?.find((u: any) => (u.email || "").toLowerCase() === String(actualEmail).toLowerCase());
